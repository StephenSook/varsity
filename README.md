...
| What-if offside-line calibrator: move the line (pointer or arrow keys) and the margin recomputes live from the same yards formula as the backend; the official call never moves | `apps/web/src/calibrate.ts` (unit-tested parity with `services/app/geometry.py`), `Demo.tsx`, the dashed what-if line in `OffsidePitch.tsx`; e2e `tests/e2e/calibrator.spec.ts` |
